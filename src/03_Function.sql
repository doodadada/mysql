...
-- 문자 관련 함수
-- SUBSTRING('문자열',시작위치, 갯수) - 문자열 중 시작위치부터 갯수만큼 출력
...
select date_add(now(), interval 3 year) as "오늘기준 3년후" from dual; 

-- DATE_SUB(날짜, INTERVAL 뺄숫자 기준값) - 날짜에서 기준값단위로 뺄 숫자만큼 밴다.


-- YEAR(날짜) -날짜의 연도 추출
-- MONTH(날짜) - 날짜의 월 추출
-- MOONTHNAME(날짜) - 날짜의 월을 영어로 추출
...
select date_format(now(), '%Y%m%d') from dual; --20240417(오늘날짜)
select str_to_date('20211013', '%Y%m%d') from dual; --20211013(입력한 문자의 날짜)
